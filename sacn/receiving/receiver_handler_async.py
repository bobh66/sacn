...
from sacn.receiving.receiver_handler_listener import ReceiverHandlerListener
from sacn.receiving.receiver_socket_base import (
    ReceiverSocketBase,
    ReceiverSocketListener,
)
from sacn.receiving.receiver_socket_udp_async import AsyncReceiverSocketUDP

E131_NETWORK_DATA_LOSS_TIMEOUT_ms = 2500


class AsyncReceiverHandler(ReceiverSocketListener):
    def __init__(self, bind_address: str, bind_port: int,
                 listener: ReceiverHandlerListener,
                 socket: ReceiverSocketBase = None):
        """
        This is a private class and should not be used elsewhere. It handles the
         receiver state with sACN specific values.
        Calls any changes in the data streams on the listener.
        Uses a UDP receiver socket with the given bind-address and -port, if the
         socket was not provided (i.e. None).
        """
        if socket is None:
            self.socket: AsyncReceiverSocketUDP = AsyncReceiverSocketUDP(self, bind_address, bind_port)
        else:
            self.socket: ReceiverSocketBase = socket
        self._listener: ReceiverHandlerListener = listener
        # previousData for storing the last data that was send in a universe to
        # check if the data has changed
        self._previousData: Dict[int, tuple] = {}
        # priorities are stored here. This is for checking if the incoming data
        # has the best priority.
        # universes are the keys and
        # the value is a tuple with the last priority and the time when this
        # priority recently was received
        self._priorities: Dict[int, tuple] = {}
        # store the last timestamp when something on an universe arrived for
    ...
    def check_for_stream_terminated_and_refresh_timestamp(self,
                                                          packet: DataPacket,
                                                          current_time: float
                                                          ) -> None:
        # refresh the last timestamp on a universe, but check if its the last
        # message of a stream
        ...
        if packet.option_StreamTerminated:
            ...
            if packet.universe not in self._lastDataTimestamps.keys():
                # fire callbacks if this is the first received packet for this
                # universe
                self._listener.on_availability_change(universe=packet.universe,
                                                      changed='available')
            self._lastDataTimestamps[packet.universe] = current_time

    def fire_timeout_callback_and_delete(self, universe: int):
        self._listener.on_availability_change(universe=universe,
                                              changed='timeout')
    ...
    def refresh_priorities(self, packet: DataPacket,
        ...
        # check if the stored priority has timeouted and make the current
        # packets priority the new one
        if packet.universe not in self._priorities.keys() or \
           ...
           self._priorities[packet.universe][0] <= packet.priority:
            # if the send priority is higher or
    ...
    def is_legal_sequence(self, packet: DataPacket) -> bool:
        ...
        For more information see page 17 of
         http://tsp.esta.org/tsp/documents/docs/E1-31-2016.pdf.
        :param packet: the packet to check
        :return: true if the sequence is legal. False if the sequence number is
         bad
        """
        # if the sequence of the packet is smaller than the last received
        ...
        try:  # try, because self.lastSequence might not been initialized
            ...
            pass
        # if the sequence is good, return True and refresh the list with the
        # new value
    ...
    def is_legal_priority(self, packet: DataPacket):
        """
        Check if the given packet has high enough priority for the stored values
         for the packet's universe.
        :param packet: the packet to check
    ...
    def fire_callbacks_universe(self, packet: DataPacket) -> None:
        # call the listeners for the universe but before check if the data
        # has changed
...
def check_timeout(current_time: float, time: float) -> bool:
    return (abs(time_millis(current_time) - time_millis(time)) >
            E131_NETWORK_DATA_LOSS_TIMEOUT_ms)
