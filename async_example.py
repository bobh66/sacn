...
async def main():
    receiver = sacn.sACNAsyncReceiver()
    await receiver.start()  # start the receiving thread
    ...
    @receiver.listen_on('universe', universe=3)  # listens on universes 1 and 2
    @receiver.listen_on('universe', universe=2)
    def callback(packet):  # packet type: sacn.DataPacket
        print(f'{packet.universe}: {packet.dmxData[:512]}')  # print the received DMX data, but only the first 8 values
